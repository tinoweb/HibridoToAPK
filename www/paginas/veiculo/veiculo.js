		...
		success: function(retorno){
			...
			if(tipo == 1){
				...
				if(retorno[0]['veiculo'][0]['foto'] == ''){
				   $( '#foto_morador_veiculo' ).html('<i class="icon material-icons" style="margin: 0px 0 0 14px; font-size: 50px; ">directions_car</i>');
				}
...
function delete_veiculo_morador(){
	...
	app2.dialog.confirm('Confirma a excluṣo','Excluir', function () {
		var dados = $( "#form_morador_veiculo" ).serialize();
		$.ajax({
			type: 'POST',
			url: localStorage.getItem('DOMINIO')+'appweb/veiculo_update.php',
